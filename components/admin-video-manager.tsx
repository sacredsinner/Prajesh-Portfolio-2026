...
export function AdminVideoManager({ videos }: { videos: Video[] }) {
  ...
  async function activate(id: number) { setBusy(true); await activateShowcaseVideo(id); window.location.reload() }
  async function remove(id: number) { if (!window.confirm("Delete this video permanently?")) return; setBusy(true); await deleteShowcaseVideo(id); window.location.reload() }
